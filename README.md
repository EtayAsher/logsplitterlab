...
Source for [logsplitterlab.com](https://logsplitterlab.com/) — a
research-based log splitter comparison site, deployed as a static site on
GitHub Pages under a custom domain (connected 2026-07-22; the repo's
underlying GitHub Pages URL, https://etayasher.github.io/logsplitterlab/,
now 301-redirects to the custom domain automatically).
...
`reviews/index.html`). Since the site is deployed at the custom domain's
root (`basePath: ''`), local preview at `http://127.0.0.1:8080/` matches
production exactly — no path adjustment needed.

## How GitHub Pages paths are handled

The site is deployed on GitHub Pages under the custom domain
`https://logsplitterlab.com/` (root), configured via the `CNAME` file at
the repo root — `tools/build.js` generates that file automatically from
`canonicalBaseUrl` in `tools/data/site-config.js`, so it always matches.
Every internal link and asset reference is generated by `layout.url(path)`
— never hand-write an `href`. If the domain ever changes again, or the
site ever needs to move back under a GitHub Pages project path (e.g.
`https://<user>.github.io/<repo>/`), update `canonicalBaseUrl` and
`basePath` in `site-config.js` and rebuild; everything else (links,
canonical tags, OG/Twitter URLs, JSON-LD, sitemap, robots.txt, the CNAME
file itself) updates from those two values alone. The repo's original
GitHub Pages URL (`https://etayasher.github.io/logsplitterlab/`) now
301-redirects to the custom domain automatically — this is standard
GitHub Pages behavior once a custom domain is configured, not something
...
## How to change the canonical base URL (e.g. moving to another domain later)
...
JSON-LD URL, sitemap entry, robots.txt Sitemap line, and internal link
updates automatically — and so does the `CNAME` file itself, which
`tools/build.js` regenerates from `canonicalBaseUrl` (only written when the
host isn't a `.github.io` domain). This is exactly the process used to
connect `logsplitterlab.com` on 2026-07-22.

## How to verify deployment

1. Push to `main`.
2. Check the repo's Actions/Pages deployment status on GitHub, or run
   `gh api repos/EtayAsher/logsplitterlab/pages/builds/latest` and confirm
   `status: "built"` for the commit you just pushed.
3. Load `https://logsplitterlab.com/` and spot-check a nested page (e.g.
   `/reviews/`) loads directly, not just via in-site navigation — this
   confirms GitHub Pages is serving directory-style `index.html` files
   correctly under the custom domain.
4. Confirm `https://etayasher.github.io/logsplitterlab/` 301-redirects to
   the custom domain (standard GitHub Pages behavior once a custom domain
   is configured — nothing to build for this, just verify it).
